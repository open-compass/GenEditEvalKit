...
def parse_args():
    ...
    parser.add_argument("--imagedir", type=str, default="./outputs")
...
class ImageCrops(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if mask is not None:
            ...
            image = self._image
        if self.args.options.get('crop', '1') == '1':
...
def _init_dist_slurm(backend, port=None, **kwargs):
    ...
    proc_id = int(os.environ['KUBEBRAIN_REPLICA'])
    ntasks = int(os.environ['PROC_PER_NODE']) * int(os.environ["NODE_COUNT"])
...
def _init_dist_slurm_2(backend, port=None, **kwargs):
    """Initialize slurm distributed training environment.
...
def main(args):
    full_results = []
    sub_folders = os.listdir(args.imagedir)
    ...
    for idx, subfolder in enumerate(sub_folders):
        if idx % 10 == 0 and rank == 0:
            print(f"[{idx}] / [{len(sub_folders)}]] - Processing {subfolder}")
...
if __name__ == "__main__":
    args = parse_args()
    _init_dist_slurm_2(backend='nccl')
