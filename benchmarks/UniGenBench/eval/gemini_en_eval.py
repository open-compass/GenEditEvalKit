...
def call_evaluation(args):
    index, prompt, testpoint, test_desc, model, img_path, api_key, base_url = args
    ...
    while True:
        ...
        response = None
        text = None
        try: # add try except to catch the error
            ...
            try:
                ...
                # If there is a response, try to print its choices attribute
                if response is not None:
                    try:
                        choices = getattr(response, "choices", None)
                        print("Response.choices (repr):", repr(choices))
                    except Exception:
                        pass
            except Exception:
                pass
...
def main(data_path: str, api_key: str, base_url: str, csv_file: str):
    ...
    for i in tqdm(range(len(df)), total=len(df)):
        ...
        for j in range(4):
            ...
            args.append((index, prompt, test_point, test_desc, model, img_path, api_key, base_url))
