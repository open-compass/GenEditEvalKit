...
for idx, row in df.iterrows():
    ...
    images = []
    # inference
    from inference import inference # Here, the model's inference function is defined as an inference interface.
    for cnt in range(grid[0] * grid[1]):
        image = inference(prompt) # As long as you define an inference interface, you can perform OneIG-Benchmark evaluation.
    ...
    total_slots = grid[0] * grid[1] # The grid is 2*2 by default, which means 4 images are needed.
    if len(images) == 0:
