...
from univa.models.qwen2p5vl.modeling_univa_qwen2p5vl_lora import UnivaQwen2p5VLLoRAForConditionalGeneration
...
def initialize_models(args, device):
    ...
    if "lora" in args.pretrained_lvlm_name_or_path:
        print(f'[INFO] Loading LoRA from {args.pretrained_lvlm_name_or_path}')
        model = UnivaQwen2p5VLForConditionalGeneration.from_pretrained(
            "/mnt/shared-storage-user/intern7shared/tianchangyao/share_ckpt_hf/UniWorld-V1", torch_dtype=torch.bfloat16
        ).to(device)
        model.denoise_tower.denoiser = PeftModel.from_pretrained(
            model.denoise_tower.denoiser, args.pretrained_lvlm_name_or_path
        ).merge_and_unload().to(device)
    else:
        print(f'[INFO] Loading model from {args.pretrained_lvlm_name_or_path}')
        model = UnivaQwen2p5VLForConditionalGeneration.from_pretrained(
            args.pretrained_lvlm_name_or_path, torch_dtype=torch.bfloat16
        ).to(device)
    # print(f'[INFO] dtype of model loaded: {next(model.parameters()).dtype}')
