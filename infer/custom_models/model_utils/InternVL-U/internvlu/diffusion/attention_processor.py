...
# This file includes code from HuggingFace,
...
class AttentionVE(nn.Module):
    ...
    def __init__(
        ...
        if qk_norm is None:
            ...
            raise ValueError(
                f"unknown qk_norm: {qk_norm}. Should be one of None, 'layer_norm', 'fp32_layer_norm', "
                "'layer_norm_across_heads', 'rms_norm', 'rms_norm_across_heads', 'l2'."
            )
        ...
        self.to_q = nn.Linear(query_dim, self.inner_dim * 2, bias=bias)
        ...
        if self.added_kv_proj_dim is not None:
            ...
            if self.context_pre_only is not None:
                self.add_q_proj = nn.Linear(
                    added_kv_proj_dim, self.inner_dim * 2, bias=added_proj_bias
                )
        ...
        if use_memory_efficient_attention_xformers:
            if is_added_kv_processor and is_custom_diffusion:
                raise NotImplementedError(
                    f"Memory efficient attention is currently not supported for custom diffusion "
            ...
            if is_custom_diffusion:
                ...
                # throw warning
                logger.info(
                    "Memory efficient attention with `xformers` might currently not work correctly "
                    "if an attention mask is required for the attention operation."
                )
